...
class NullSink(SinkElement):
    """A sink that does precisely nothing. It is useful for testing and
    debugging, or for pipelines that do not need a sink, but require one to be
...
class CollectSink(SinkElement):
    """A sink element that has one collection per sink pad. Each frame that is
    pulled into the sink is added to the collection for that pad using a
    ".append" method. If the extract_data flag is set, the data is extracted
    from the frame and added to the deque , otherwise the frame itself is added
    to the collection.

    Args:
        collects:
            dict[str, MutableSequence], a mapping of sink pads to mutable
            sequences, where the key is the pad name and the value is the
            sequence. The sequence must have an append method.
        extract_data:
    ...
    Notes:
        Ignoring empty frames:
            ...
            principle is that "empty frames preserve the sink deque". An empty
            deque is equivalent (for our purposes) to a deque filled with
            "None" values, so we prevent the latter from being possible.
    """

    collects: Optional[dict[str, MutableSequence[Any]]] = None
    ...
    def __post_init__(self):
        ...
        for pad_name in self.collects:
            if pad_name not in self.sink_pad_names_full:
                raise ValueError(
                    "DequeSink has a iterable for a pad that does not exist, "
    ...
    def pull(self, pad: SinkPad, frame: Frame) -> None:
        ...
        if frame.data is not None:
            assert self.collects is not None
